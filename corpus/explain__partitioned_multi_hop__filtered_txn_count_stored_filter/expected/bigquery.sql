...
-- Select: ['__filtered_txn_count']
-- Aggregate Inputs for Simple Metrics
-- Compute Metrics via Expressions
-- Write to DataTable
SELECT
  SUM(filtered_txn_count) AS filtered_txn_count
FROM (
  -- Join Standard Outputs
  -- Select: ['__filtered_txn_count', 'account_id__customer_id__customer_name']
  SELECT
    subq_11.customer_id__customer_name AS account_id__customer_id__customer_name
    , account_month_txns_src_10000.txn_count AS filtered_txn_count
  ...
  LEFT OUTER JOIN (
    ...
    SELECT
      DATETIME_TRUNC(bridge_table_src_10000.ds_partitioned, day) AS ds_partitioned__day
    ...
    ON
      (
        ...
        DATETIME_TRUNC(bridge_table_src_10000.ds_partitioned, day) = DATETIME_TRUNC(customer_table_src_10000.ds_partitioned, day)
      )
  ) subq_11
  ON
    (
      ...
      DATETIME_TRUNC(account_month_txns_src_10000.ds_partitioned, day) = subq_11.ds_partitioned__day
    )
) subq_13
